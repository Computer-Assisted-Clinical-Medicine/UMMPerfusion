...
#import "UMMPAFOneAlgorithmController.h"



@implementation UMMPAFOneAlgorithmController
...
- (void)changeROIRec:(UMMPROIRec *)roiRec
{
	...
	[arterialMenuItem setTitle:[NSString stringWithFormat:@"%@   SL:%ld   TP:%ld", [[roiRec roi] name],
                                (long)[roiRec slice]+1, (long)[roiRec timePoint]+1]];
    [controller changeView:allROIsTag];
...
- (void)removeROIRec:(UMMPROIRec *)roiRec
{
	...
	if (!recordsExist) {
		...
		[tissueROITableView setEnabled:NO];
...
- (void)drawSelectedROIRecs
{
	...
	for (UMMPROIRec *roiRec in records) {
		...
		if ([roiRec tag] == [[arterialButton selectedItem] tag] || [[roiRec activated] boolValue] || (([roiRec tag] == [[controller roiList] externalRoiRecTag]) && ([[arterialButton selectedItem] tag] == externalROITag))) {
...
-(BOOL) checkUserInput
{
    ...
    int i, counter=0;
	for(i=0; i<[[[controller roiList] records]count]; i++){
		
		if ([[[[[controller roiList] records] objectAtIndex:i] activated] boolValue]) {
			counter++;
		}
	}
	if(!counter) {
...
-(void)saveInputParameter:(UMMPROIRec *)tissueROI andAlgorithmName:(NSString *)algorithm
{
	...
	[inputParameter addObject:[NSMutableDictionary dictionaryWithObject:[[algorithmButton selectedItem] title] forKey:@"Algorithm:"]];
...
- (void)savePresetParameter:(int)tag
{
    ...
    switch ([algorithmButton selectedTag]) {
        case 0:
...
// returns the number of ROIs in the recordsForAllRoisAlgorithm list
- (NSInteger)numberOfRowsInTableView:(NSTableView *)tableView
{
	if (tableView == tissueROITableView) {
		return [[[controller roiList] recordsForAllRoisAlgorithm] count];
	} 
	
	return 0;
}

// creates the tissue list from the recordsForAllRoisAlgorithm list
- (id)tableView:(NSTableView *)tableView
objectValueForTableColumn:(NSTableColumn *)tableColumn
            row:(NSInteger)row
{
	if (tableView == tissueROITableView) {
		NSString *name = [NSString stringWithFormat:@"%@   SL: %ld   TP: %ld", [[[[[controller roiList] recordsForAllRoisAlgorithm] objectAtIndex:row] roi] name], (long)[[[[controller roiList] recordsForAllRoisAlgorithm] objectAtIndex:row] timePoint]+1 , (long)[[[[controller roiList] recordsForAllRoisAlgorithm] objectAtIndex:row] slice]+1];
		
		[tissueButtonCell setTitle:name ];
        
		id param = [[[[controller roiList] recordsForAllRoisAlgorithm] objectAtIndex:row] activated];
...
{
    if (aTableView == tissueROITableView) {
        [[[[controller roiList] recordsForAllRoisAlgorithm] objectAtIndex:rowIndex] setActivated:anObject];
        [tissueROITableView setNeedsDisplay:YES];
    }
}
