...
#import "UMMPRoiBasedController.h"


@implementation UMMPRoiBasedController
...
// Test if its viable to always use 2D TimeArray for calculation ( remember to rename Method Calls in UMMPPanelController to the old one if not )
    ...
    for(i=0; i<n-1; i++){
        ...
        time[i+1] = [viewerTimeArray[i+start][slice] doubleValue];
        NSLog(@"UMMPRoiBasedController: given timeArray[%d][%d]= %lf",i+start ,slice ,[viewerTimeArray[i+start][slice] doubleValue] );
    ...
    // switch between the ROI based algorithms
    switch ([[controller algorithmPopUpButton] selectedTag]) {
        case compartmentRoiTag:
    ...
    int selectedAlgorithmTag = [[controller algorithmPopUpButton] selectedTag];
	 ...
	 [dict setObject:[tracerButton titleOfSelectedItem] forKey:@"tracer"];
        ...
        if (selectedAlgorithmTag == filtrationRoiTag ) {
    ...
    //next 3 if()'s all concern Permeable surface area product,(mathematically the same), just the displayed names differ
        if (selectedAlgorithmTag == exchangeRoiTag || selectedAlgorithmTag == uptakeRoiTag) {
        ...
        if (selectedAlgorithmTag == modifiedToftsRoiTag) {
